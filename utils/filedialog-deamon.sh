...
PID=$1
needtoexport=0;
echo "" > /home/furios/.cache/signalut.pparent/exportlock
allreadynavigated=0
  ...
  # flush pending events pour éviter d'accumuler
        ...
        for window in $windows ; do
            ...
            if [ "$prop" = 'WM_WINDOW_ROLE(STRING) = "GtkFileChooserDialog"' ]; then
                ...
                if [ "$window_min_height" -gt "753" ]; then
                    echo "Export File"
                    for file in /home/furios/.cache/signalut.pparent/downloads/* ; do
                        utils/rm.sh $file
                    done
                    ...
                    xdotool sleep 5
                    while read -t 0.01 -r _; do :; done
                    echo "">/home/furios/.cache/signalut.pparent/downloads/00000000.png
                    utils/rm.sh /home/furios/.local/share/signalut.pparent/recently-used.xbel
                fi
            fi
        done    
    fi
    ...
    if [ "$needtoexport" -ne "0" ]; then
                ...
                if [ "$?" -eq "1" ]; then
                    ...
                    echo "download file" 
                     read lock < /home/furios/.cache/signalut.pparent/exportlock
                    if [ "$lock" != "lock" ]; then
                        echo "lock" > /home/furios/.cache/signalut.pparent/exportlock
                       ( qmlscene utils/download-helper/qml/ExportPage.qml -I  utils/download-helper/; echo "" >/home/furios/.cache/signalut.pparent/exportlock)  &
                       xdotool sleep 5;
                       echo "">/home/furios/.cache/signalut.pparent/downloads/00000000.png
                       utils/rm.sh /home/furios/.local/share/signalut.pparent/recently-used.xbel
                    fi
